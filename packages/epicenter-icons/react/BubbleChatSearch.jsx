import React from 'react';

const BubbleChatSearch = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M21.5 12C21.5 17.2467 17.2467 21.5 12 21.5C10.3719 21.5 8.8394 21.0904 7.5 20.3687C5.5 19.5 3.5 19.9125 2.5 20C2.98894 19.1667 3.60922 16.8818 2.98341 15C2.6698 14.057 2.5 13.0483 2.5 12C2.5 6.75329 6.75329 2.5 12 2.5C17.2467 2.5 21.5 6.75329 21.5 12Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M16 15.4998L14.375 13.8748M15 11.4998C15 9.84292 13.6569 8.49977 12 8.49977C10.3431 8.49977 9 9.84292 9 11.4998C9 13.1566 10.3431 14.4998 12 14.4998C12.8284 14.4998 13.5784 14.164 14.1213 13.6211C14.6642 13.0782 15 12.3282 15 11.4998Z" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

BubbleChatSearch.displayName = 'BubbleChatSearch';

export default BubbleChatSearch;
