import React from 'react';

const UngroupItems = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M6 4C6 5.10457 5.10457 6 4 6C2.89543 6 2 5.10457 2 4C2 2.89543 2.89543 2 4 2C5.10457 2 6 2.89543 6 4Z" />
    <path d="M22 4C22 5.10457 21.1046 6 20 6C18.8954 6 18 5.10457 18 4C18 2.89543 18.8954 2 20 2C21.1046 2 22 2.89543 22 4Z" />
    <path d="M22 20C22 21.1046 21.1046 22 20 22C18.8954 22 18 21.1046 18 20C18 18.8954 18.8954 18 20 18C21.1046 18 22 18.8954 22 20Z" />
    <path d="M6 20C6 21.1046 5.10457 22 4 22C2.89543 22 2 21.1046 2 20C2 18.8954 2.89543 18 4 18C5.10457 18 6 18.8954 6 20Z" />
    <path d="M15.5 7.5H8.5C7.94772 7.5 7.5 7.94772 7.5 8.5V9.5C7.5 10.0523 7.94772 10.5 8.5 10.5H15.5C16.0523 10.5 16.5 10.0523 16.5 9.5V8.5C16.5 7.94772 16.0523 7.5 15.5 7.5Z" strokeLinejoin="round" />
    <path d="M15.5 13.5H8.5C7.94772 13.5 7.5 13.9477 7.5 14.5V15.5C7.5 16.0523 7.94772 16.5 8.5 16.5H15.5C16.0523 16.5 16.5 16.0523 16.5 15.5V14.5C16.5 13.9477 16.0523 13.5 15.5 13.5Z" strokeLinejoin="round" />
  </svg>
);

UngroupItems.displayName = 'UngroupItems';

export default UngroupItems;
