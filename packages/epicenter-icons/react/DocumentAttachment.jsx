export const DocumentAttachment = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M11 22H5.50002C4.39544 22 3.50001 21.1046 3.50002 20L3.50016 3.99998C3.50017 2.89542 4.39559 2 5.50016 2H18.5C19.6046 2 20.5 2.89543 20.5 4V10.5" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M7.5 7H16.5" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M7.5 12H13.5" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M20.5 20L20.5 17C20.5 15.5706 19.1569 14 17.5 14C15.8431 14 14.5 15.5706 14.5 17L14.5 20.5C14.5 21.3284 15.1716 22 16 22C16.8284 22 17.5 21.3284 17.5 20.5V17" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

DocumentAttachment.displayName = 'DocumentAttachment';
