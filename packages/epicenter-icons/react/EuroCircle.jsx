import React from 'react';

const EuroCircle = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12Z" strokeLinejoin="round" />
    <path d="M15.5 9.5C15 8.5 14.1936 8 13.1056 8C11.6665 8 10.5 9.16655 10.5 10.6056V13.3944C10.5 14.8335 11.6665 16 13.1056 16C14.1936 16 15 15.5 15.5 14.5M8.5 12H13.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

EuroCircle.displayName = 'EuroCircle';

export default EuroCircle;
