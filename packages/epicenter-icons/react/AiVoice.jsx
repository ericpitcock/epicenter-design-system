import React from 'react';

const AiVoice = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M9 11V14" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M11.5 3H5C3.34315 3 2 4.34315 2 6V19C2 20.6569 3.34315 22 5 22H18C19.6569 22 21 20.6569 21 19V12.5" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M12 8V17" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M15 10V15" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M6 12V13" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M15.3881 5.08714C16.796 4.91193 17.9119 3.79602 18.0871 2.38812C18.1137 2.17498 18.2852 2 18.5 2C18.7148 2 18.8863 2.17498 18.9129 2.38812C19.0881 3.79602 20.204 4.91193 21.6119 5.08714C21.825 5.11366 22 5.28522 22 5.5C22 5.71478 21.825 5.88634 21.6119 5.91286C20.204 6.08807 19.0881 7.20398 18.9129 8.61188C18.8863 8.82502 18.7148 9 18.5 9C18.2852 9 18.1137 8.82502 18.0871 8.61188C17.9119 7.20398 16.796 6.08807 15.3881 5.91286C15.175 5.88634 15 5.71478 15 5.5C15 5.28522 15.175 5.11366 15.3881 5.08714Z" />
  </svg>
);

AiVoice.displayName = 'AiVoice';

export default AiVoice;
