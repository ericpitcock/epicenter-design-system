import React from 'react';

const CircleArrowMoveDownRight = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" strokeLinejoin="round" />
    <path d="M13 16L15 14L13 12" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M9 8V12C9 13.1046 9.89543 14 11 14H15" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

CircleArrowMoveDownRight.displayName = 'CircleArrowMoveDownRight';

export default CircleArrowMoveDownRight;
