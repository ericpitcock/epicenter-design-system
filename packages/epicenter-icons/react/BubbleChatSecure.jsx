export const BubbleChatSecure = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M21.5 12C21.5 17.2467 17.2467 21.5 12 21.5C10.3719 21.5 8.8394 21.0904 7.5 20.3687C5.5 19.5 3.5 19.9125 2.5 20C2.98894 19.1667 3.60922 16.8818 2.98341 15C2.6698 14.057 2.5 13.0483 2.5 12C2.5 6.75329 6.75329 2.5 12 2.5C17.2467 2.5 21.5 6.75329 21.5 12Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M15.5 9.2C14.675 8.46801 13.4136 8 12 8C10.5864 8 9.32498 8.46801 8.5 9.2V11.6C8.5 14.8 12 16 12 16C12 16 15.5 14.8 15.5 11.6L15.5 9.2Z" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

BubbleChatSecure.displayName = 'BubbleChatSecure';
