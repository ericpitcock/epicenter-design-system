export const UmbrellaDollar = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M12 3.5V2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M12 3.5C16.9706 3.5 21 6.97715 21 12.5H3C3 6.97715 7.02944 3.5 12 3.5Z" strokeLinecap="square" strokeLinejoin="round" />
    <path d="M12 3.5C14.2091 3.5 16 6.97715 16 12.5H8C8 6.97715 9.79086 3.5 12 3.5Z" strokeLinecap="square" strokeLinejoin="round" />
    <path d="M12 12.5V20.5C12 21.3284 11.3284 22 10.5 22C9.67157 22 9 21.3284 9 20.5V20" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M17.5 16C16.6716 16 16 16.5596 16 17.25C16 17.9404 16.6716 18.5 17.5 18.5C18.3284 18.5 19 19.0596 19 19.75C19 20.4404 18.3284 21 17.5 21M17.5 16C17.9907 16 18.4264 16.1964 18.7001 16.5M17.5 16L17.5 15M17.5 21C17.0093 21 16.5736 20.8036 16.2999 20.5M17.5 21V22" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

UmbrellaDollar.displayName = 'UmbrellaDollar';
