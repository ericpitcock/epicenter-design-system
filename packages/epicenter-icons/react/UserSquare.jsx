export const UserSquare = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M15 10C15 8.34315 13.6569 7 12 7C10.3431 7 9 8.34315 9 10C9 11.6569 10.3431 13 12 13C13.6569 13 15 11.6569 15 10Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M17 18C17 15.2386 14.7614 13 12 13C9.23858 13 7 15.2386 7 18" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M3 4.89474C3 3.8483 3.8483 3 4.89474 3H19.1053C20.1517 3 21 3.8483 21 4.89474V19.1053C21 20.1517 20.1517 21 19.1053 21H4.89474C3.8483 21 3 20.1517 3 19.1053V4.89474Z" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

UserSquare.displayName = 'UserSquare';
