export const PlayListFavourite01 = (props) => (
  <svg
    className="ep-icon"
    viewBox="0 0 24 24"
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path d="M3 7H21" strokeLinejoin="round" />
    <path d="M17 2.5L14 7" strokeLinejoin="round" />
    <path d="M10 2.5L7 7" strokeLinejoin="round" />
    <path d="M10.5 21.5H4.5C3.39543 21.5 2.5 20.6046 2.5 19.5V4.5C2.5 3.39543 3.39543 2.5 4.5 2.5H19.5C20.6046 2.5 21.5 3.39543 21.5 4.5V10.5" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M20.086 13.831C18.2898 12.8795 17 14.2821 17 14.2821C17 14.2821 15.7102 12.8795 13.9139 13.831C11.7383 14.9834 11.5821 19.4964 17 21.5C22.4179 19.4964 22.2616 14.9834 20.086 13.831Z" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

PlayListFavourite01.displayName = 'PlayListFavourite01';
